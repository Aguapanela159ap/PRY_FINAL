...
static const float3 WallColor         = float3(1.0, 0.5, 0.0);
static const float3 PlayerColor       = float3(0.0, 2.0, 0.0);
static const float3 AmbientLightColor = float3(0.7, 0.0, 0.5) * 0.7;
static const float3 FlashLightColor   = float3(1.0, 1.0, 1.0) * 0.6;
static const float3 AmbientLight      = float(0.001).xxx;
static const float3 TeleportColor     = float3(0.0, 0.2, 0.5);
static const float3 TeleportWaveColor = float3(0.5, 0.2, 0.7);
static const float3 SpikesColor       = float3(1.0, 0.0, 0.0); // Red for spikes
static const float3 SpikesWaveColor   = float3(1.0, 0.5, 0.0); // Orange wave for spikes
...
float4 PSmain(in PSInput PSIn) : SV_TARGET
{
    ...
    const float  DistToSpikes   = distance(g_MapConstants.SpikesPos, PosOnMap);
    ...
    // draw spikes
    {
        float Factor = saturate(1.0 - DistToSpikes / g_MapConstants.SpikesRadius);
        Color.rgb    = Blend(Color.rgb, SpikesColor, Factor);
    }
    ...
    // draw spikes wave
    {
        const float WaveWidth = 0.2;
        float       Shading   = 1.0 - smoothstep(0.0, 0.0001, dot(Color.rgb, Color.rgb)); // hide wave if it is illuminated
        float       Wave      = BumpStep(DistToSpikes, g_MapConstants.SpikesWaveRadius, g_MapConstants.SpikesWaveRadius + WaveWidth);
        Color.rgb             = Blend(Color.rgb, SpikesWaveColor, Wave * Shading);
    }
