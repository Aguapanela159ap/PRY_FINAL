...
private:
    ...
    {
        float2 Pos; // pixels
        float2 FlashLightDir   = {10.0f, 10.0f};
        float  FlashLightPower = 1.0f; // 0 - off, 1 - max brightness
        ...
        // Nuevos par�metros para la luz ambiental
        float3 AmbientLightColor  = {1.2f, 1.2f, 1.2f}; // Color de la luz ambiental (aj�stalo como desees)
        float  AmbientLightRadius = 360.0f;
        float  VertVel            = 0.0f;  // velocidad vertical (+ sube, − baja)
        bool   JumpRequested      = false; // bandera puesta por KeyEvent()
    ...
    struct
    {
        const float PlayerRadius          = 0.2f;   // pixels, must be less than 0.5, because we use 1 bit SDF
        const float AmbientLightRadius    = 360.0f; // pixels
        const float FlshLightMaxDist      = 100.0f; // pixels
        const float PlayerVelocity        = 30.0f;  // pixels / second
        const float FlashLightAttenuation = 200.0f; // power / second
