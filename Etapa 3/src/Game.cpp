...
// -----------------------------------------------------------------------------
// Game::Update – movimiento lateral, gravedad, salto y lógica extra
// -----------------------------------------------------------------------------
void Game::Update(float dt)
{
    ...
    ////////////////////////////////////////////////////////////////////////////
    // BLOQUE 1 – Movimiento horizontal (A / D)                                //
    ////////////////////////////////////////////////////////////////////////////
    {
        float HorizontalDir   = m_Player.PendingPos.x; // -1 = izq, +1 = der
        m_Player.PendingPos.x = 0.0f;                  // consumimos la entrada

        if (std::abs(HorizontalDir) > 0.1f)
        {
            const float2 StartPos = m_Player.Pos;
            const float2 EndPos   = m_Player.Pos +
                float2{HorizontalDir, 0.0f} *
                    dt * Constants.PlayerVelocity;
            ...
            auto ReadMap = [&](int x, int y) -> float {
                ...
                return 1.0f; // fuera del mapa = pared
            };
            ...
            for (Uint32 i = 0; i < Constants.MaxCollisionSteps; ++i)
            {
                ...
                float dist = lerp(lerp(c00, c10, fract(FetchPos.x)),
                                  lerp(c01, c11, fract(FetchPos.x)),
                                  fract(FetchPos.y));

                if (dist > Constants.PlayerRadius)
                    break; // punto libre encontrado

                m_Player.Pos = Pos; // aplicamos posición válida
            }

            // teleport si toca el portal
            if (length(m_Map.TeleportPos - m_Player.Pos) < Constants.TeleportRadius)
                LoadNewMap();
        }
    }                         // ← variables locales de este bloque quedan fuera de alcance aquí
    m_Player.PendingPos = {}; // limpiamos cualquier otra dirección


    ////////////////////////////////////////////////////////////////////////////
    // BLOQUE 2 – Física vertical (gravedad + salto con W)                    //
    ////////////////////////////////////////////////////////////////////////////
    {
        ...
        auto IsGroundBelow = [&](float2 pos) -> bool {
            ...
            int         y      = static_cast<int>(pos.y - Constants.PlayerRadius - 0.05f);
            if (x < 0 || x >= static_cast<int>(TexDim.x) ||
                y < 0 || y >= static_cast<int>(TexDim.y))
                return true; // borde = pared
            return m_Map.MapData[x + y * TexDim.x];
        };
        ...
        else
        {
            ...
            m_Player.VertVel = std::max(m_Player.VertVel,
                                        -Constants.TerminalFallSpeed);
        }

        const float2 StartPos = m_Player.Pos;
        const float2 EndPos   = m_Player.Pos +
            float2{0.0f, m_Player.VertVel * dt};
        ...
        m_Player.JumpRequested = false; // salto consumido
    }


    ////////////////////////////////////////////////////////////////////////////
    // BLOQUE 3 – Resto de lógica (linterna, animaciones, etc.)               //
    ////////////////////////////////////////////////////////////////////////////
    {
        // dirección de la linterna
        ...
        float2      SNormMousePos =
            (m_Player.MousePos /
             uint2{SCDesc.Width, SCDesc.Height}.Recast<float>()) *
                2.0f -
            float2(1.f, 1.f);
        ...
        m_Player.FlashLightPower =
            clamp(m_Player.FlashLightPower +
                      Constants.FlashLightAttenuation * (m_Player.LMBPressed ? dt : -dt),
                  20.0f, 20.0f);
...
void Game::Draw()
{
    ...
    // update player constants
    {
        ...
        //Const.AmbientLightColor  = m_Player.AmbientLightColor;
        Const.FlshLightMaxDist = Constants.FlshLightMaxDist * 400.5f;
...
void Game::KeyEvent(Key key, KeyState state)
{
    if (state == KeyState::Press || state == KeyState::Repeat)
    {
        switch (key)
        {
            ...
            // ------------- SALTO con W ---------------------
            case Key::W:
                if (state != KeyState::Release) // Press o Hold
                    m_Player.JumpRequested = true;
                break;

            // (opcional) seguir usando flecha ↑ para “subir” cámaras/menús
            case Key::Up:
            case Key::NP_Up:
...
void Game::GenerateMap()
{
    ...
    //  ░  Zig-zag 4 platas
    //      y = altura del muro
    //      0 = fondo   TexDim.y-1 = techo
    //
    //  Planta 0 (más alta) → hueco a la DERECHA
    //  Planta 1            → hueco a la IZQUIERDA
    //  Planta 2            → hueco a la DERECHA
    //  Planta 3 (más baja) → hueco a la DERECHA
    // -----------------------------------------------------------------------------
    {
        ...
        for (Uint32 i = 0; i < NumFloors; ++i)
        {
            ...
            //  i = 0→D, 1→I, 2→D, 3→I  ▸ alternamos con (i & 1)
            Uint32 HoleStart = (i & 1) ? // impar → izquierda
                1 :
                TexDim.x - 1 - HoleWidth; // par  → derecha

            for (Uint32 x = 1; x < TexDim.x - 1; ++x)
            {
                if (x >= HoleStart && x < HoleStart + HoleWidth)
                    continue; // dejar hueco
                MapData[x + y * TexDim.x] = true;
            }
        }
    }
    // -----------------------------------------------------------------------------
    //  Fin del zig-zag – sigue el código original (clear center, teleport, etc.)
    // -----------------------------------------------------------------------------
