...
void Game::Update(float dt)
{
    ...
    if (PosDeltaLen > 0.1f)
    {
        ...
        for (Uint32 i = 0; i < Constants.MaxCollisionSteps; ++i)
        {
            ...
            // Desactivar las colisiones para la linterna
            if (m_Player.LMBPressed)
            {
                // Aquí no verificamos la colisión de la linterna con las paredes
                continue; // La linterna atraviesa los muros
            }
    ...
    m_Player.FlashLightPower = clamp(m_Player.FlashLightPower + Constants.FlashLightAttenuation * (m_Player.LMBPressed ? dt : -dt), 20.0f, 20.0f);
...
void Game::GenerateMap()
{
   ...
   // Eliminar la creación de muros aleatorios
    {
        // Esta parte genera los muros aleatorios, simplemente la eliminamos
